...
% probabilistic model of the objective function (e.g., validation \acrshort{map}) and using an acquisition function to decide which hyperparameters to evaluate next. This approach attempts to balance exploration (trying new, uncertain areas) and exploitation (focusing on areas known to yield good results).
