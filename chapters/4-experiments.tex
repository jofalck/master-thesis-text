...
The large difference in validation \acrshort{map}@50 for \acrshort{tdeed} could result from a lack of postprocessing. \acrshort{tdeed} uses two different functions for predicting the test \acrshort{map} and the validation \acrshort{map}. The test evaluator applies soft non-maximum suppression to remove redundant detections, while the validation does not remove redundant predictions. In addition, the validation predicts with a stride of \(2\), but the test has a stride of \(1\). With \(fps=25\) and a \(tolerance = \pm 1s\), this reduction in temporal resolution hinders the predictions from hitting the tolerance window. 
...
% Training multiple models with the same preprocessed data with tdeed is very slow. 

% preprocessing vms splits videos into 1 min clips AND runs them through model (very parallelizable)

% faster with a100 in feat extraction

% In Table tab:results_ex2, VMS inference is listed as 0.1 min (7s). However, the subsequent paragraph states: "Inference time for one game is approximately the same time as the preprocessing of that game, in this case (342min)". Which value is correct for the model's forward pass on pre-extracted features? Does the 342 min refer to a scenario where features must be extracted during inference for a new, unseen game?
...
% Table tab:results_ex2 lists TDEED inference as 5 min. The text calculates total inference as 13 min (preprocessing) + 5 min (model) = 18 min. Is the 5 min in the table only the model's forward pass time, assuming features are already extracted?
...
% The setup mentions A100 GPUs for the main experiment. Table tab:average_feature_extraction (VMAE preprocessing for VMS) lists V100s and one P100. Was the "Preprocessing (1 game)" for VMS (342 min) in Table tab:results_ex2 also run on V100s, or was it on an A100? How does this potential hardware difference affect the direct comparison of preprocessing times between TDEED (13 min, presumably on A100) and VMS (342 min)?
% You noted an \unsure about using A100s. Given the results, how does the choice of GPU impact the narrative about VMS being runnable on "any GPU" versus TDEED needing "premium GPUs"?
...
% You had an \unsure note: "should I use identical epoch count and batch size?". Were the epoch counts and batch sizes identical or comparable (e.g., training until convergence) for TDEED and VMS in this experiment? If not, how might this have influenced the training times?
...
% The setup states, "A100 \acrshort{gpu}s are used, of size 80 GB or 40 GB, with an indifference to interpret the results." Why was there indifference? Could the memory difference have impacted runtimes, even slightly, and should this be acknowledged as a minor limitation or factor?
...
% You mention, "full parallelizability is assumed in preprocessing and inference." How was this ensured or measured? What are the implications if true parallelization was not perfectly achieved?
...
% What are the primary computational bottlenecks for each model during (a) preprocessing, (b) training, and (c) inference? (e.g., CPU-bound, GPU-bound, I/O-bound).



% How do these runtime differences relate to the accuracy findings in Experiment 1? Is there a clear speed-accuracy trade-off, or does one model excel in both?
...
% VMS preprocessing is significantly longer. In what scenarios is this a one-time cost versus a recurring one? How does this affect the overall "time-to-result" for different use cases (e.g., retraining frequently vs. deploying a static model)?

% For Table tab:average_feature_extraction, you mention the Stoke City game time was a "measuring error." What led to this conclusion, as opposed to it being an actual longer processing time due to the P100 GPU or specific video content?
