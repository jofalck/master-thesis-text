...
\unsure{ActionFormer results are not discussed. It isn't able to generalize the data, it seems, and uses more time than VMS. Twice as much on training and 1min 20sec on inference.}
...
Experiment 7 compares ActionFormer to \acrshort{vms}. The only difference between the model is that \acrshort{vms} has switched out the transformer blocks to mamba blocks.
...
There is a big difference between the validation and the test accuracy of the ActionFormer model. I believe this is because the transformer operates on too little data and is therefore unable to generalize effectively. Both models exhibit a similar drop from training to validation, suggesting that neither suffers from overfitting. This aligns with the theory, which indicates that Mamba performs better on smaller datasets. 
